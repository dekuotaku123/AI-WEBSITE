...
import { BotIcon, Divide, EyeIcon, Loader2Icon, SendIcon, UserIcon } from "lucide-react";
...
const Sidebar = ({
  ...
  return (
    <div
      className={`h-full sm:max-w-sm rounded-xl bg-gray-900 border-gray-800 transistion-all ${isMenuOpen ? "max-sm:w-0 overflow-hidden" : "w-full"}`}
    >
      <div className="flex flex-col h-full">
            ...
            .map((message) => {
              ...
              if (isMessage) {
                ...
                return (
                  <div
                    key={msg.id}
                    className={`flex item-center gap-3 ${isUser ? "justify-end" : "justify-starat"}`}
                  >
                    {!isUser && (
                    ...
                    <div
                      className={`max-w-[80%] p-2 px-4 rounded-2xl shadow-sm text-sm mt-5 leading-relaxed ${isUser ? "bg-linear-t-r from-indigo-500 to-indigo-600 text-white rounded-tr-none" : "rounded-tl-none bg-gray-800 text-gray-100"}`}
                    >
                      {msg.content}
                    </div>
                    {!isUser && (
        ...
        {/* input area */}
        <form onSubmit={handleRevisions} className="relative m-3" action="">
          <div className="flex items-center gap-2">
            ...
            />
            <button disabled={isGenerating||!input.trim()} className="absolute bottom-2.5 right-2.5 rounded-full bg-linear-to-r from indgo-500 to-indigo-600 hover:from-indigo-600 hover:to-indigo-700 text-white transistion-colors">
